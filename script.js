...
function save(){
  let arr =Array.from(document.querySelectorAll('.work'));
  if(arr.length>0){
    info.innerHTML=`Today's Work`;
  }
  else{
    localStorage.clear();
    info.innerHTML=`No Works Today`;
  }
  let works=[];
  arr.forEach((e)=>{
    works.push(e.innerHTML);
  })
